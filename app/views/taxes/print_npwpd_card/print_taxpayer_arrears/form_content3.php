		  					...
		  					<select name="input4-wp_wr_detil_id" id="input4-wp_wr_detil_id" class="form-control" required>
		  						...
		  						<?php
		  							foreach($taxpayer_detail_rows as $row){
		  								echo "<option value='".$row['wp_wr_detil_id']."'>".$row['nama']." (".$row['nama_kegus'].")</option>";
		  							}
		  						?>
		  					</select>
		  				</div>
...
<script type="text/javascript">
    ...
    function delete_taxobject_image(id){
        ...
        ajax_object.set_url(base_url+'bundle/taxes/hotel/record_taxpayer1/delete_taxobject_image')
