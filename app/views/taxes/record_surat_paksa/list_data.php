	...
	<tbody>
		...
		foreach ($rows as $row) {
			...
			echo "<tr>
					...
					<td align='center'>" . $row['srt_paksa_periode'] . "</td>
					<td>" . mix_2Date($row['srt_paksa_periode_jual1'], $row['srt_paksa_periode_jual2']) . "</td>
					<td>" . $row['npwpd'] . " / <br />" . $row['nama_wp'] . "</td>
					<td align='right'>" . number_format($row['pajak']) . "</td>
					<td align='center'>" . indo_date_format($row['srt_paksa_jatuh_tempo'], 'longDate') . "</td>
					<td align='center'>" . $row['srt_paksa_kode_billing'] . "</td>
					<td align='center'>";

			if ($print_priv == '1') {
				echo "<a href='" . base_url() . "bundle/" . $bundle_type . "/" . $bundle_item_type . "/" . $menu . "/_print?id=" . $row['srt_paksa_id'] . "' id='btn-print" . $no . "' 
			...
			echo "
						<input type='hidden' id='ajax-req-dt' name='id' value='" . $row['srt_paksa_id'] . "'/>
