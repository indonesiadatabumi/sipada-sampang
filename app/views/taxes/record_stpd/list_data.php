	...
	<tbody>
		...
		foreach ($rows as $row) {
			...
			echo "<tr>
					...
					<td align='center'>" . $row['stpd_periode'] . "</td>
					<td>" . mix_2Date($row['stpd_periode_jual1'], $row['stpd_periode_jual2']) . "</td>
					<td>" . $row['npwpd'] . " / <br />" . $row['nama_wp'] . "</td>
					<td align='right'>" . number_format($row['pajak']) . "</td>
					<td align='center'>" . indo_date_format($row['stpd_jatuh_tempo'], 'longDate') . "</td>
					<td align='center'>" . $row['stpd_kode_billing'] . "</td>
					<td align='center'>";

			if ($print_priv == '1') {
				echo "<a href='" . base_url() . "bundle/" . $bundle_type . "/" . $bundle_item_type . "/" . $menu . "/_print?id=" . $row['stpd_id'] . "' id='btn-print" . $no . "' 
			...
			echo "
						<input type='hidden' id='ajax-req-dt' name='id' value='" . $row['stpd_id'] . "'/>
