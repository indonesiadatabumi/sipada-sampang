...
header("Content-Disposition: attachment; filename=Daftar SPTPD $tax_name.xls");
?>
<!DOCTYPE html>
<html>
				...
				foreach ($rows as $row) {
					...
					if ($jenis_pajak == '7') {
						...
						echo "<tr>
								...
								<td align='right'>" . (!empty($row['nilai_terkena_pajak']) ? number_format($row['nilai_terkena_pajak'], 2, '.', ',') : '') . "</td>
								<td align='right'>" . (!empty($row['pajak']) ? number_format($row['pajak'], 2, '.', ',') : '') . "</td>
							</tr>";
					}
				}
				?>
