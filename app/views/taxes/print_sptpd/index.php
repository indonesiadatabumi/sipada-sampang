					...
					<form class="form-horizontal" id="search_form" action="<?= base_url() . "/bundle/taxes/" . $bundle_item_type . "/" . $menu . "/load_list"; ?>" method="POST">
						...
						<fieldset>
							...
							<div class="form-group">
								...
								<div class="col-md-2">
									<div class="input">
										<input type="text" class="form-control" name="tgl_cetak" value="<?= date('d-m-Y'); ?>" id="tgl_cetak">
									</div>
								</div>
							</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		$("#src-tgl_proses_awal,#src-tgl_proses_akhir,#tgl_cetak").mask('99-99-9999');

		// START AND FINISH DATE AND TGL CETAK
		$("#src-tgl_proses_awal,#src-tgl_proses_akhir,#tgl_cetak").datepicker({
			dateFormat: 'dd-mm-yy',
	...
	function show_all_data() {
		ajax_object.reset_object();
		data_ajax = ['menu=' + $('#menu').val(), 'src-pajak_id=' + $('#bundle_id').val(), 'tgl_cetak=' + $('#tgl_cetak').val()];
	...
	function load_taxpayer_search_panel(id) {
		...
		if (showed == '2') {
			...
			data_ajax.push("tgl_cetak=" + $('#tgl_cetak').val());
		}
