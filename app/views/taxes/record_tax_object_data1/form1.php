<form id="<?= $main_form_id; ?>" method="POST" action="<?= base_url() . "/bundle/" . $bundle_type . "/" . $bundle_item_type . "/" . $menu . "/submit_form"; ?>" class="form-horizontal">
    ...
    <input type="hidden" name="pajak_id" value="<?= $bundle_id; ?>" />
                ...
                <div class="panel panel-primary">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <div class="input">
                                    <input class="form-control" value="<?= ($act == 'add' ? $tax_year : $curr_data['tahun_pajak']); ?>" readonly />
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        if (in_array($bundle_id, $dtl_type1_arr)) {
                            ...
                            if ($bundle_id == 4) {
                                ...
                                echo "<table class='table table-bordered'>
                                    <thead>
                                        <tr><th>Mineral Bukan Logam & Batuan</th><th width='20%'>Volume (m<sup>3</sup>)</th><th width='20%'>Tarif Dasar</th><th width='20%'>Nilai Jual</th><th>-</th></tr>
                                    </thead>
                                ...
                                if ($act == 'add') {
                                    ...
                                    foreach ($nonmetalic_mineral_rock_type_rows as $row) {
                                        echo "<option value='" . $row['ref_mblb_id'] . "_" . $row['tarif_dasar'] . "'>" . $row['jenis_mblb'] . "</option>";
                                    }
                                    echo "</select>
                                                </td>
                                                <td>
                                    ...
                                    foreach ($nonmetalic_mineral_rock_rows as $row1) {
                                        ...
                                        foreach ($nonmetalic_mineral_rock_type_rows as $row2) {
                                            ...
                                            echo "<option value='" . $row2['ref_mblb_id'] . "_" . $row2['tarif_dasar'] . "' " . $selected . ">" . $row2['jenis_mblb'] . "</option>";
                                        }
                                        echo "</select>
                                                    </td>
                                                    <td>
...
<script type="text/javascript">
    ...
    function assess_tax2() { //groundwater tax
        ...
        // console.log(compensation_component_weights);
        $tbody.empty();
    ...
    function add_detail2_row() { //nonmetallic_mineral_rock tax
        ...
        new_row = "<tr id='row-" + new_order + "'><td><select class='form-control' name='input2-mblb_id" + new_order + "' onchange=\"mix_function3(" + new_order + ")\" id='input2-mblb_id" + new_order + "'>";
        new_row += "<option value=''></option>";

        for (i = 0; i < nonmetalic_mineral_rock_type_rows.length; i++) {
            new_row += "<option value='" + nonmetalic_mineral_rock_type_rows[i]['ref_mblb_id'] + "_" + nonmetalic_mineral_rock_type_rows[i]['tarif_dasar'] + "'>" + nonmetalic_mineral_rock_type_rows[i]['jenis_mblb'] + "</option>";
        }
