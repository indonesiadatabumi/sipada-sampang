		...
		<table class="report" cellpadding="0" cellspacing="0">
			<thead>
				<tr>
					...
					<?php
					if ($bundle_id == 6) {
						echo "<th>Penerimaan Kabupaten</th>";
						echo "<th>Penerimaan Provinsi</th>";
					}
					?>
				</tr>
			</thead>
			<tbody>
				...
				$total_pen_prov = 0;
				foreach ($rows as $row) {
					$opsen_mblb = (25/100)*$row['pokok_pajak'];
					$penerimaan_kab = (80/100)*$row['pokok_pajak'];
					$penerimaan_prov = (20/100)*$row['pokok_pajak'];
					$no++;
					if (count($judul_kegus) > 0) {
						if ($judul_kegus['nama_kegus'] == 'Jasa Boga / Katering dan Sejenisnya') {
							echo "<tr>
							...
							if ($bundle_id == 6) {
								...
								<td align='right'>" . number_format($row['denda'], 0, ',', '.') . "</td>
								<td align='right'>" . number_format($row['total_bayar'], 0, ',', '.') . "</td>
								<td align='right'>" . number_format($penerimaan_kab, 0, ',', '.') . "</td>
								<td align='right'>" . number_format($penerimaan_prov, 0, ',', '.') . "</td>
							</tr>";
							}else {
								echo "<tr>
								<td align='center'>" . $no . "</td>
					...
					$total_pen_kab += $penerimaan_kab;
					$total_pen_prov += $penerimaan_prov;
				}
				?>
			</tbody>
			<tfoot>
					...
					<td align='right'><b>" . number_format($total, 0, ',', '.') . "</b></td>
						...
						if ($bundle_id == 6) {
							echo "<tr>
								...
								<td align='right'><b>" . number_format($total_denda, 0, ',', '.') . "</b></td>
								<td align='right'><b>" . number_format($total, 0, ',', '.') . "</b></td>
								<td align='right'><b>" . number_format($total_pen_kab, 0, ',', '.') . "</b></td>
								<td align='right'><b>" . number_format($total_pen_prov, 0, ',', '.') . "</b></td>
							</tr>";
						}else {
							echo "<tr>
								<td colspan='6' align='center'><b>JUMLAH</b></td>
