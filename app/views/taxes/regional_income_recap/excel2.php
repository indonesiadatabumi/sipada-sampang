						...
						<tr>
							...
							<td>
								<table class="noborder">
									...
									<tr>
										<td>BULAN</td>
										<td>: <?= strtoupper($periode_report) . " " . $tax_year; ?></td>
									</tr>
								</table>
							</td>
						</tr>
		...
		<table class="report" cellpadding="0" cellspacing="0">
			<thead>
				<tr>
					...
					<td align="center" colspan="2"><b>REALISASI S.D BULAN LALU</td>
					<td align="center" colspan="2"><b>REALISASI BULAN INI</td>
					<td align="center" colspan="2"><b>REALISASI S.D BULAN INI</td>
			...
			<tbody>
				...
				foreach ($rows1 as $row1) {
        			...
        			LEFT JOIN (SELECT SUM(total_bayar) as tot_realisasi1, a.pajak_id 
								...
								WHERE (to_char(tgl_bayar, 'mm-yyyy') < '" . $process_date . "-" . $tax_year . "') AND b.kelurahan_id='" . $row1['kelurahan_id'] . "' GROUP BY a.pajak_id) as c ON (a.bundel_id=c.pajak_id) 
        			LEFT JOIN (SELECT SUM(total_bayar) as tot_realisasi2, a.pajak_id 
        						FROM transaksi_pajak as a
								LEFT JOIN wp_wr_detil AS b ON a.wp_wr_detil_id=b.wp_wr_detil_id
								WHERE (to_char(tgl_bayar, 'mm-yyyy') = '" . $process_date . "-" . $tax_year . "') AND b.kelurahan_id='" . $row1['kelurahan_id'] . "' GROUP BY a.pajak_id) as d ON (a.bundel_id=d.pajak_id) 
