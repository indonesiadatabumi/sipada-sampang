<?php
	...
	class print_taxpayer_closure_list extends item_bundle_parent{
		...
		function show_report($type){
			...
			$date_period = "";
			if(isset($src_params['tgl_tutup_awal']) && isset($src_params['tgl_tutup_akhir'])){
				$date_period = "Periode Tanggal ".mix_2Date(us_date_format($src_params['tgl_tutup_awal']),us_date_format($src_params['tgl_tutup_akhir']));
			}else if(isset($src_params['tgl_tutup_awal'])){
				$date_period = "Periode Tanggal ".mix_2Date(us_date_format($src_params['tgl_tutup_awal']),date('Y-m-d'));
			}else if(isset($src_params['tgl_tutup_akhir'])){
				$date_period = "S.D Tanggal ".indo_date_format(us_date_format($src_params['tgl_tutup_akhir']),'longDate');
			}			
		...
		function get_rows($src_params){
			...
			$cond = " b.pajak_id='".$this->bundle_id."'";
			...
			if(isset($src_params['kecamatan_id'])){
				$cond .= (!empty($cond)?" AND ":"")." b.kecamatan_id='".$src_params['kecamatan_id']."'";
			}			

			if(isset($src_params['tgl_tutup_awal']) && isset($src_params['tgl_tutup_akhir']))
			{
				$cond .= (!empty($cond)?" AND ":"")." tgl_tutup BETWEEN '".us_date_format($src_params['tgl_tutup_awal'])."' AND '".us_date_format($src_params['tgl_tutup_akhir'])."'";
			}else if(isset($src_params['tgl_tutup_awal'])){
				$cond .= (!empty($cond)?" AND ":"")." tgl_tutup >= '".us_date_format($src_params['tgl_tutup_awal'])."'";
			}else if(isset($src_params['tgl_tutup_akhir'])){
				$cond .= (!empty($cond)?" AND ":"")." tgl_tutup <= '".us_date_format($src_params['tgl_tutup_akhir'])."'";
			}
