...
class record_surat_paksa extends item_bundle_parent
{
	...
	function load_taxpayer_list()
	{
		...
		$sql = "SELECT a.*, b.stpd_id, c.st_id, d.srt_paksa_id
				FROM v_spt2 a
				...
				LEFT JOIN surat_paksa d ON c.st_kode_billing=d.srt_paksa_kode_billing
				WHERE a.status_bayar = '0' AND pajak_id = '" . $this->bundle_id . "' AND b.stpd_id IS NOT NULL AND c.st_id IS NOT NULL AND d.srt_paksa_id IS NULL ORDER BY spt_id DESC";
	...
	function form()
	{
		...
		$sql = "SELECT * FROM surat_paksa WHERE srt_paksa_id='" . ($id_value == '' ? '0' : $id_value) . "'";
	...
	function submit_form()
	{
		...
		$query = "select max(srt_paksa_id)+1 as srt_paksa_id from surat_paksa";
		$result = $this->dao->execute(0, $query)->row_array();
		$srt_paksa_id = $result['srt_paksa_id'];
		if ($srt_paksa_id == null) {
			$srt_paksa_id = 1;
		}

		$query_st_id = "select st_id from surat_teguran2 where st_kode_billing = '$kode_billing'";
		$result_st_id = $this->dao->execute(0, $query_st_id)->row_array();
		...
		if ($access == '1') {
			...
			if ($act == 'add') {
				$st_id = $result_st_id['st_id'];
				$query = "INSERT INTO surat_paksa (srt_paksa_id, srt_paksa_jenis_pajak, srt_paksa_tgl_proses, srt_paksa_jatuh_tempo, srt_paksa_periode, srt_paksa_nomor, srt_paksa_wp_id, srt_paksa_korek_id, srt_paksa_periode_jual1, srt_paksa_periode_jual2, srt_paksa_kode_billing, no_surat, st_id) VALUES ($srt_paksa_id, $pajak_id, '$tgl_proses', '$jatuh_tempo', $periode, $srt_paksa_id, $wp_wr_id, '$korek_id', '$masa_pajak1', '$masa_pajak2', '$kode_billing', '$no_surat', $st_id)";
	...
	function delete_record()
	{
		...
		if ($access == '1') {
			...
			$query = "DELETE FROM surat_paksa WHERE srt_paksa_id=$id";
			$result = $this->dao->execute(0, $query);
			$label = "menghapus";
	...
	function _print()
	{
		...
		$sql = "SELECT a.*, b.nama_pajak, b.nomor_spt, b.npwpd, b.nama_wp, b.alamat, b.nama_pemilik, b.alamat_pemilik, b.persen_tarif, b.nilai_terkena_pajak, b.pajak, b.jenis_spt_id, c.no_surat as no_surat_st FROM surat_paksa as a 
				LEFT JOIN v_spt2 as b ON (a.srt_paksa_kode_billing=b.kode_billing)
				LEFT JOIN surat_teguran2 as c ON (a.st_id=c.st_id)
				WHERE a.srt_paksa_id='" . $id . "'";
		$rows2 = $this->dao->execute(0, $sql)->row_array();

		$data['rows'] = $rows2;

		$denda = $this->get_fine($rows2['pajak'], $rows2['srt_paksa_jatuh_tempo'], date('Y-m-d'), $rows2['jenis_spt_id'], $jenis_pajak);

		$data['denda'] = $denda;

		$data['grand_total'] = $rows2['pajak'] + $denda;

		$view_folder = $this->bundle_type . '/record_surat_paksa';
