<?php
	...
	class taxpayer_closure extends item_bundle_parent{
		...
		function load_taxpayer_list(){
			...
			$sql = "SELECT a.wp_wr_detil_id,a.utama,b.npwprd,c.nama_paret,d.nama_kegus,a.nama,a.alamat,a.kelurahan,a.kecamatan 
					...
					WHERE a.pajak_id='".$this->bundle_id."' AND a.status=TRUE AND (b.npwprd LIKE '%".$key."%' OR a.nama LIKE '%".$key."%')";
		...
		function submit_form(){
			...
			if($access=='1'){
				...
				$input_params2 = array('status'=>'FALSE','tgl_tutup'=>$input_params['tgl_tutup']);
				...
				if($taxpayer_detail_row['utama']=='TRUE'){
					...
					$input_params3 = array('status'=>'FALSE');
		...
		function delete_record(){
			...
			if($access=='1'){
				...
				$input_params = array('status'=>'TRUE','tgl_tutup'=>'');
				...
				if($taxpayer_detail_row['utama']=='TRUE'){
					$this->_ci->load->model('wp_wr_model');
					$m3 = $this->_ci->wp_wr_model;
					$input_params2 = array('status'=>'TRUE');
