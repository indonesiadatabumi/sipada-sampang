...
class record_tax_object_data extends item_bundle_parent
{
	...
	function form()
	{
		...
		} elseif ($this->bundle_id == 14) {
			$sql = "SELECT a.*, b.kapasitas_daya, b.koefisien_daya, b.waktu_pemakaian, b.tarif_dasar FROM spt_detil AS a
						LEFT JOIN spt_detil_penerangan_jalan AS b ON a.spt_id=b.spt_id 
						WHERE a.spt_id='" . ($id_value == '' ? '0' : $id_value) . "'";
			$curr_data2 = $this->dao->execute(0, $sql)->row_array();
		} else {
			$sql = "SELECT spt_detil_id FROM spt_detil WHERE spt_id='" . ($id_value == '' ? '0' : $id_value) . "'";
		...
		if ($this->bundle_id == '3') {
			...
			if ($skip_register == '1') {
				array_unshift($district_rows, array(
					'kecamatan_id' => '9999999',
					'dt2_id' => '9999',
					'nama_kecamatan' => 'LUAR KOTA',
					'kode_kecamatan' => '00'
				));
			}
			...
			if ($act == 'add') {
				...
				if ($this->bundle_id == '3') {
					...
					if ($this->bundle_id == 14) {
						...
						$input_params3['kapasitas_daya'] = str_replace(',', '', $_POST['input2-kapasitas_daya']);
						$input_params3['koefisien_daya'] = str_replace(',', '', $_POST['input2-koefisien_daya']);
						$input_params3['waktu_pemakaian'] = str_replace(',', '', $_POST['input2-waktu_pemakaian']);
						$input_params3['tarif_dasar'] = str_replace(',', '', $_POST['input2-tarif_dasar']);
						$input_params3['nilai_terkena_pajak'] = $input_params['nilai_terkena_pajak'];
