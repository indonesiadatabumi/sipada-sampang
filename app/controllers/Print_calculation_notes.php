<?php
	...
	class print_calculation_notes extends item_bundle_parent{
		...
		function show_report($report_type){			
			...
			// var_dump($system_params);
			// die();
			foreach ($rows as $tes) {
				$sql2 = "SELECT * FROM v_spt as a LEFT JOIN wp_wr as b on (a.wp_wr_id=b.wp_wr_id)WHERE npwpd='".$tes['npwprd']."' ";

				$spt = $this->dao->execute(0,$sql2)->row_array();}
			...
			if($spt_type_id=='1'){
				$view_folder .= ($this->bundle_id!='3'?'1':'2');
			}else{
				$view_folder .= '3';
			}
		...
		function get_rows($src_params){
			...
			if($spt_type_id=='1'){
				...
				if($this->bundle_id!='3'){
					$sql = "SELECT a.penetapan_id,a.tgl_penetapan,a.kohir,b.*,c.singkatan as singkatan_spt FROM penetapan_pajak as a 
							INNER JOIN (SELECT a.*,
										b.npwprd as npwpd,c.nama as nama_wp,c.alamat,c.kelurahan,c.kecamatan,
										d.dasar_hukum,d.kode_rekening,d.nama_kegus,d.volume,d.diskon,d.denda,d.bunga FROM spt as a 
								...
								LEFT JOIN ref_kegiatan_usaha as y ON (x.kegus_id=y.ref_kegus_id)) as d ON (a.spt_id=d.spt_id)) as b ON (a.spt_id=b.spt_id) 
							LEFT JOIN ref_jenis_spt as c ON (a.jenis_spt_id=c.ref_jenspt_id)";
				}else{
					$sql = "SELECT a.penetapan_id,a.tgl_penetapan,a.kohir,b.* FROM penetapan_pajak as a 
							INNER JOIN ( SELECT a.*,b.jenis_reklame_id,b.area,b.judul,b.lokasi, 
										b.ukuran as luas,b.jumlah,b.jangka_waktu as lama_pasang,b.harga_satuan as nilai_tarif,b.nilai_sewa_reklame as nsr, 
										b.persen_tarif,b.jenis_reklame, c.nama as nama_wp,c.alamat,c.kelurahan,c.kecamatan, 
										d.dasar_hukum,d.kode_rekening FROM spt as a 
							LEFT JOIN (SELECT x.*,y.jenis_reklame FROM spt_detil_reklame as x LEFT JOIN ref_jenis_reklame as y 
							           ON (x.jenis_reklame_id=y.ref_jenrek_id)) as b ON (a.spt_id=b.spt_id) 
							LEFT JOIN wp_wr_reklame as c ON (a.wp_wr_id=c.wp_wr_reklame_id) 
							LEFT JOIN (SELECT x.spt_id,y.dasar_hukum,y.kode_rekening FROM spt_detil as x 
							           LEFT JOIN ref_kegiatan_usaha as y ON (x.kegus_id=y.ref_kegus_id)) as d 
							            ON (a.spt_id=d.spt_id)) as b ON (a.spt_id=b.spt_id)  ";
				}
				...
				$sql = "SELECT a.penetapan_id,a.tgl_penetapan,a.kohir,b.*,c.singkatan as singkatan_spt FROM penetapan_pajak as a 
						INNER JOIN (SELECT a.*,b.npwprd as npwpd,c.nama as nama_wp,c.alamat,c.kelurahan,c.kecamatan,
									d.dasar_hukum,d.kode_rekening,d.nama_kegus,e.nilai_terkena_pajak  FROM laporan_hasil_pemeriksaan as a 
									LEFT JOIN wp_wr as b ON (a.wp_wr_id=b.wp_wr_id) 
									LEFT JOIN wp_wr_detil as c ON (a.wp_wr_detil_id=c.wp_wr_detil_id)
			...
			$tax_year = (isset($src_params['tahun_pajak'])?$src_params['tahun_pajak']:date('Y'));
			$cond = " a.pajak_id='".$this->bundle_id."' AND a.tahun_pajak='".$tax_year."' AND ";
